...
AXES = [
    {
        ...
        # key 는 화면(index.html)의 번역 사전을 찾는 이름표다. label 은 폴백용.
        "zones": [("fng.exfear", "극단적 공포", 0, 25, -2),
                  ("fng.fear", "공포", 25, 45, -1),
                  ("fng.neutral", "중립", 45, 55, 0),
                  ("fng.greed", "탐욕", 55, 75, 1),
                  ("fng.exgreed", "극단적 탐욕", 75, 101, 2)],
    ...
    },
    {
        ...
        # 구간 경계는 시장에서 통용되는 둥근 숫자에 맞췄다(20 / 30 / 40).
        # 백분위로 보면 20은 중간(약 63%) 언저리라 '불안'이라 부르기엔 이른 편이지만,
        # 남들이 쓰는 기준과 어긋나면 오히려 헷갈린다는 판단으로 관행을 따랐다.
        # 참고 백분위 — 12:하위10%, 15:30%, 20:63%, 30:92%, 40:98%.
        "zones": [("vix.verycalm", "아주 잠잠", 0, 12, 2),
                  ("vix.calm", "잠잠", 12, 15, 1),
                  ("vix.normal", "보통", 15, 20, 0),
                  ("vix.anxious", "불안", 20, 30, -1),
                  ("vix.fear", "공포", 30, 40, -2),
                  ("vix.panic", "패닉", 40, 200, -2)],
        "help": ("VIX는 앞으로 한 달 주가가 얼마나 출렁일지 시장이 내다보는 정도라서 "
                 "'공포지수'라고 부릅니다. 시장에서는 보통 20을 평상시와 불안의 "
                 "경계로, 30 이상을 스트레스, 40 이상을 위기로 봅니다. "
                 "다만 1990년 이후 절반이 17.6 아래였으니 20은 평균보다 조금 "
                 "높은 정도입니다. 공포탐욕지수와 반대로 오른쪽이 공포입니다."),
    },
]
    ...
    result = {
        ...
        "axes": [{**{k: a[k] for k in
                     ("key", "label", "short", "title", "window",
                      "lo", "hi", "step", "dec", "help")},
                  "zones": [{"key": k, "label": l, "lo": lo, "hi": hi, "tone": t}
                            for k, l, lo, hi, t in a["zones"]]}
